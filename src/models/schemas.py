...
class Query(BaseModel):
    ...
    embedding_provider: Optional[str] = Field(
        default=None, description="Embedding provider to use"
    )
    rerank_method: Optional[str] = Field(
        default=None, description="Reranking method to use"
    )
...
class RAGRequest(BaseModel):
    ...
    n_results: int = Field(
        default=10, description="Number of results to retrieve", ge=1, le=100
    )
    where: Optional[Dict[str, Any]] = Field(default=None, description="Filter criteria")
    ...
    temperature: Optional[float] = Field(
        default=None, description="Temperature for the LLM", ge=0.0, le=1.0
    )
    llm_provider: Optional[str] = Field(default=None, description="LLM provider to use")
    embedding_provider: Optional[str] = Field(
        default=None, description="Embedding provider to use"
