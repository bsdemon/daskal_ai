...
class VectorDB:
    ...
    async def add_documents(self, documents: List[str], metadatas: List[Dict[str, Any]] = None) -> List[str]:
...
class ContextualVectorDB(VectorDB):
    ...
    def __init__(
        ...
        self.contextual_embedding_enabled = settings.ENABLE_CONTEXTUAL_EMBEDDING and settings.ENABLE_EMBEDDING
        ...
        self.context_cache = self._load_cache() if self.contextual_embedding_enabled else {}
    ...
    async def add_documents(self, documents: List[str], metadatas: List[Dict[str, Any]] = None) -> List[str]:
        ...
        if self.contextual_embedding_enabled and self.embedding_enabled and self.llm_client:
            enhanced_docs = []
            enhanced_metadatas = []
    ...
    async def search(
        ...
        if self.reranking_enabled and rerank_method and results:
            ...
            original_texts = [result["metadata"].get("original_text", result["text"]) for result in results]
            ...
            try:
                ...
                for doc, score in reranked:
                    ...
                    for result in results:
                        if result["metadata"].get("original_text", result["text"]) == doc:
