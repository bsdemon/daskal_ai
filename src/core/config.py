...
class Settings(BaseSettings):
    ...
    # API Configuration
    API_PRESHARED_KEY: str = Field(..., description="Preshared API key for authentication")
    ...
    # Embedding Service
    VOYAGE_API_KEY: Optional[str] = Field(None, description="Voyage AI API key for embeddings")
    
    # Reranking Service
    COHERE_API_KEY: Optional[str] = Field(None, description="Cohere API key for reranking")
    
    # Database Configuration
    CHROMA_PERSIST_DIRECTORY: str = Field("./chroma_db", description="ChromaDB persistence directory")
    
    # Feature Flags
    ENABLE_EMBEDDING: bool = Field(True, description="Enable embedding service")
    ENABLE_CONTEXTUAL_EMBEDDING: bool = Field(True, description="Enable contextual descriptions for embeddings")
    ...
    DEFAULT_LLM_PROVIDER: str = Field("anthropic", description="Default LLM provider: anthropic, openai, or gemini")
    DEFAULT_EMBEDDING_PROVIDER: str = Field("voyage", description="Default embedding provider")
    DEFAULT_RERANKING_METHOD: str = Field("bm25", description="Default reranking method: bm25, cohere")
