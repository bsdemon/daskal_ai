...
class RerankerClient(ABC):
    ...
    @abstractmethod
    async def rerank(self, query: str, documents: List[str], scores: List[float] = None) -> List[Tuple[str, float]]:
...
class BM25Reranker(RerankerClient):
    ...
    def _compute_idf(self, query_tokens: List[str], documents: List[str]) -> Dict[str, float]:
    ...
    async def rerank(self, query: str, documents: List[str], scores: List[float] = None) -> List[Tuple[str, float]]:
        ...
        avg_doc_len = sum(len(self.tokenizer(doc)) for doc in documents) / len(documents)
        ...
        for i, doc in enumerate(documents):
            ...
            for token in query_tokens:
                ...
                denominator = tf[token] + self.k1 * (1 - self.b + self.b * doc_len / avg_doc_len)
...
class CohereReranker(RerankerClient):
    ...
    async def rerank(self, query: str, documents: List[str], scores: List[float] = None) -> List[Tuple[str, float]]:
        ...
        reranked_docs = [(documents[result.index], result.relevance_score) for result in response.results]
